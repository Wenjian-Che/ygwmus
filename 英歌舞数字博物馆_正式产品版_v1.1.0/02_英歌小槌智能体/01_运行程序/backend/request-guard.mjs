...
export function validateAgentRequest(body, customLimits = {}) {
  ...
  const value = { message, history };
  if (typeof body.app_id === "string" && cleanText(body.app_id)) value.app_id = cleanText(body.app_id).slice(0, 64);
  if (typeof body.conversation_id === "string" && cleanText(body.conversation_id)) value.conversation_id = cleanText(body.conversation_id).slice(0, 100);
  if (body.context && typeof body.context === "object" && !Array.isArray(body.context)) {
    const context = {};
    for (const key of ["audience", "detail_level", "source_mode", "page", "topic"]) {
      if (typeof body.context[key] === "string" && cleanText(body.context[key])) context[key] = cleanText(body.context[key]).slice(0, 120);
    }
    if (Object.keys(context).length) value.context = context;
  }
  if (body.client && typeof body.client === "object" && !Array.isArray(body.client) && typeof body.client.locale === "string") {
    value.client = { locale: cleanText(body.client.locale).slice(0, 24) };
  }
