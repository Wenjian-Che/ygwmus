...
export async function createStore(backendDir, { requirePersistent = false } = {}) {
  let DatabaseSync;
  try {
    ({ DatabaseSync } = await import("node:sqlite"));
  } catch (error) {
    if (requirePersistent) {
      throw Object.assign(new Error("生产环境需要支持 node:sqlite 的 Node.js 运行时"), { code: "PERSISTENT_STORE_UNAVAILABLE", cause: error });
    }
