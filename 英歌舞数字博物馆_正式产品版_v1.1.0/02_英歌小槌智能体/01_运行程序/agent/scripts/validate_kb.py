...
from build_rag import AGENT, KNOWLEDGE, ROOT, build, load_json, terms
...
def validate_structure() -> list[str]:
    ...
    actual = sorted(p.name for p in KNOWLEDGE.glob("[0-9][0-9]_*.md"))
    if sorted(listed) != actual:
        errors.append(f"manifest mismatch: listed={len(listed)} actual={len(actual)}")
    for filename in listed:
        text = (KNOWLEDGE / filename).read_text(encoding="utf-8-sig")
        for target in re.findall(r"\[\[([^\]|#]+)", text):
            candidate = KNOWLEDGE / (target if target.endswith(".md") else target + ".md")
    ...
    facts = (KNOWLEDGE / "50_核心事实注册表.md").read_text(encoding="utf-8-sig")
