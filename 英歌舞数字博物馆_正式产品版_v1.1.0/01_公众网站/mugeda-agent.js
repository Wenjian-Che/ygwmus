(function () {
  ...
  ready(function () {
    ...
    var activeSpeech = null, activeSpeechUrl = '', activeSpeechButton = null;
    function stopSpeech(button) { var currentButton = button || activeSpeechButton; if (activeSpeech) { activeSpeech.pause(); activeSpeech = null; } if (activeSpeechUrl) { URL.revokeObjectURL(activeSpeechUrl); activeSpeechUrl = ''; } if (currentButton) { currentButton.disabled = false; currentButton.classList.remove('reading'); currentButton.textContent = '朗读回答'; } activeSpeechButton = null; resumePageAudio(); }
    ...
    async function ask(question) {
      ...
      finally { send.disabled = false; resumeMugedaWakeIfEnabled(); if (window.matchMedia('(pointer:fine)').matches) input.focus(); }
    }
    ...
    var mugedaWakeEnabled = false, mugedaWakeArmedThisVisit = false, mugedaAutoSpeechEnabled = false;
    ...
    wakeSetting.onclick = function () { if (!canCloudVoice || !window.isSecureContext) { setStatus('当前页面无法开启语音唤醒', true); return; } if (mugedaWakeEnabled && !mugedaWakeArmedThisVisit) mugedaWakeArmedThisVisit = true; else { mugedaWakeEnabled = !mugedaWakeEnabled; mugedaWakeArmedThisVisit = mugedaWakeEnabled; try { localStorage.setItem(MUGEDA_WAKE_STORAGE_KEY, mugedaWakeEnabled ? '1' : '0'); } catch (_) {} } refreshMugedaSettings(); if (mugedaWakeEnabled && mugedaWakeArmedThisVisit) startMugedaWakeStandby(); else stopMugedaWakeStandby({ quiet: false }); };
    speechSetting.onclick = function () { mugedaAutoSpeechEnabled = !mugedaAutoSpeechEnabled; try { localStorage.setItem(MUGEDA_AUTO_SPEECH_STORAGE_KEY, mugedaAutoSpeechEnabled ? '1' : '0'); } catch (_) {} refreshMugedaSettings(); };
    ...
    var MUGEDA_WAKE_COOLDOWN_MS = 1800;
    var MUGEDA_WAKE_REQUEST = { mode: 'wake' };
    var mugedaWakeSessionId = '', mugedaWakeRecorder = null, mugedaWakeQueue = Promise.resolve(), mugedaWakeStarting = false, mugedaWakeLastTriggerAt = 0, mugedaWakeAck = null;
    ...
    function stopMugedaWakeStandby(options) { options = options || {}; var recorder = mugedaWakeRecorder, session = mugedaWakeSessionId; mugedaWakeRecorder = null; mugedaWakeSessionId = ''; mugedaWakeStarting = false; mugedaWakeQueue = Promise.resolve(); if (recorder) recorder.cancel(); deleteMugedaVoiceSession(session); launcher.classList.remove('listening'); if (!options.quiet && !mugedaWakeQuestionActive) setStatus('页面唤醒已关闭'); }
    function enqueueMugedaWakeChunk(chunks, inputRate) { var session = mugedaWakeSessionId; if (!session) return; mugedaWakeQueue = mugedaWakeQueue.catch(function () {}).then(function () { if (session !== mugedaWakeSessionId) return null; return postMugedaVoiceChunk(session, chunks, inputRate, false); }).then(function (payload) { if (!payload || !payload.awake) return; if (Date.now() - mugedaWakeLastTriggerAt < MUGEDA_WAKE_COOLDOWN_MS) { reportMugedaVoiceEvent('wake_duplicate_suppressed'); return; } mugedaWakeLastTriggerAt = Date.now(); handleMugedaWakeDetected(payload); }).catch(function (error) { stopMugedaWakeStandby({ quiet: true }); setStatus(error.message || '页面唤醒暂时不可用', true); reportMugedaVoiceEvent('voice_error'); }); }
    function startMugedaWakeStandby() { if (!mugedaWakeEnabled || !mugedaWakeArmedThisVisit || document.hidden || !canCloudVoice || mugedaWakeStarting || mugedaWakeRecorder || mugedaWakeQuestionActive) return; mugedaWakeStarting = true; setStatus('正在开启页面唤醒'); Promise.all([navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 1 } }), createMugedaVoiceSession(MUGEDA_WAKE_REQUEST.mode)]).then(function (values) { var stream = values[0], payload = values[1]; mugedaWakeStarting = false; if (!mugedaWakeEnabled || !mugedaWakeArmedThisVisit || document.hidden) { stream.getTracks().forEach(function (track) { track.stop(); }); deleteMugedaVoiceSession(payload.session_id); return; } mugedaWakeSessionId = payload.session_id; mugedaWakeQueue = Promise.resolve(); mugedaWakeRecorder = beginMugedaPcmRecorder(stream, enqueueMugedaWakeChunk); launcher.classList.add('listening'); setStatus('正在等待“小槌小槌”'); reportMugedaVoiceEvent('wake_started', { engine: payload.engine || 'mugeda-local-kws' }); }).catch(function (error) { mugedaWakeStarting = false; setStatus(error.message || '页面唤醒暂时不可用，可使用点按说话', true); reportMugedaVoiceEvent('voice_error'); }); }
    function playMugedaWakeAcknowledgement() { if (mugedaWakeAck) { try { mugedaWakeAck.pause(); } catch (_) {} } return new Promise(function (resolve) { var finished = false, done = function () { if (finished) return; finished = true; window.clearTimeout(timer); resolve(); }; mugedaWakeAck = new Audio(mugedaVoiceUrl('/assets/voice/xiaochui-wake-response.wav?v=1.2.2')); mugedaWakeAck.onended = done; mugedaWakeAck.onerror = done; var timer = window.setTimeout(done, 1800), playback = mugedaWakeAck.play(); if (playback && playback.catch) playback.catch(done); }); }
    function handleMugedaWakeDetected(payload) { stopMugedaWakeStandby({ quiet: true }); stopSpeech(); suspendPageAudio(); open(); setStatus('小槌我在'); reportMugedaVoiceEvent('wake_detected', { engine: payload.engine || 'mugeda-local-kws' }); playMugedaWakeAcknowledgement().then(function () { startMugedaWakeQuestion(); }); }
    ...
    window.MugedaYinggeAgent = { open: open, close: shut, ask: ask, setScene: function (scene) { config.scene = scene; }, version: '1.5.0' };
