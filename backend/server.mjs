...
function retrieve(query, limit = 8, intent = null) {
  ...
  const staticResults = chunks.map((chunk) => {
...
function systemPrompt() {
  return `你是“英歌有意思”的英歌舞文化知识助手。只能依据随后提供的证据回答；证据没有说的内容要明确说待核，不要补造。必须区分通用机制、具体地区和具体队伍。来源事实、视频观察和分析推断要分层。不要把一支队伍的动作、鼓点、人数或阵法说成全国统一标准。回答先给直接结论，再给机制、具体例子、适用边界和下一步核验方法。不要输出HTML、脚本、API密钥或虚构URL。`;
}
...
function appendApprovedKnowledge(candidate) {
  ...
  const knowledgePath = path.join(projectRoot, "84_审核通过知识补充.md");
  ...
  const trackedFiles = ["84_审核通过知识补充.md", "agent/generated/chunks.jsonl", "agent/generated/lexical_index.json", "agent/generated/build_report.json", "web/data/chunks.jsonl", "web/data/lexical_index.json", "web/data/build_report.json"];
...
async function handleChat(request, response, body) {
  ...
  if (!body.message || typeof body.message !== "string") return sendJson(response, 400, { code: "INVALID_REQUEST", message: "message 不能为空" });
  if (body.message.length > 8000) return sendJson(response, 400, { code: "MESSAGE_TOO_LONG", message: "问题长度不能超过 8000 字符" });
  ...
  try {
    ...
    return sendJson(response, 503, { code: "MODEL_UNAVAILABLE", message: "模型服务暂时无法连接", detail: String(error?.message || error) });
  }
  if (!upstream.ok) return sendJson(response, upstream.status === 401 ? 502 : 503, { code: "MODEL_UNAVAILABLE", upstream_status: upstream.status, message: await upstream.text() });
  ...
  sendEvent(response, "meta", { message_id: messageId, app_id: appId, model: prepared.payload.model, intent: prepared.intent.key, intent_label: prepared.intent.label, knowledge_version: knowledgeVersion });
  ...
  if (prepared.evidenceQuality.key !== "supported") {
    store.recordUnanswered({ app_id: appId, question: body.message, intent: prepared.intent.key, quality: prepared.evidenceQuality.key });
  }
  store.recordInteraction({ message_id: messageId, app_id: appId, question: body.message, answer: sanitizeModelAnswer(answerText), intent: prepared.intent.key, quality: prepared.evidenceQuality.key, citation_count: prepared.citations.length, citation_files: [...new Set(prepared.evidence.map((item) => item.source_file).filter((item) => item && !String(item).startsWith("[动态]")))] });
  sendEvent(response, "done", { message_id: messageId, confidence: prepared.evidenceQuality.label, intent: prepared.intent.key, intent_label: prepared.intent.label, evidence_quality: prepared.evidenceQuality.key, knowledge_version: knowledgeVersion });
  response.end();
}
...
async function handleTest(response, body) {
  ...
  if (!body.message) return sendJson(response, 400, { code: "INVALID_REQUEST", message: "测试问题不能为空" });
  ...
  const raw = await upstream.text();
  if (!upstream.ok) return sendJson(response, 502, { code: "MODEL_UNAVAILABLE", upstream_status: upstream.status, message: raw });
  let parsed;
  try { parsed = JSON.parse(raw); } catch { return sendJson(response, 502, { code: "INVALID_MODEL_RESPONSE" }); }
  sendJson(response, 200, { answer: sanitizeModelAnswer(parsed.choices?.[0]?.message?.content || ""), model: parsed.model, intent: prepared.intent, evidence_quality: prepared.evidenceQuality, citations: prepared.citations, usage: parsed.usage || null });
}
...
const server = http.createServer(async (request, response) => {
  ...
  response.setHeader("access-control-allow-headers", "content-type, authorization, x-app-id");
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/admin/config") return sendJson(response, 200, { model: config.model, base_url: config.baseUrl, thinking: config.thinking, api_key_configured: Boolean(process.env.DEEPSEEK_API_KEY), admin_auth_enabled: Boolean(config.adminToken), knowledge_version: knowledgeVersion });
    ...
    if (request.method === "GET" && url.pathname === "/api/admin/metrics") return sendJson(response, 200, { ...store.metrics(), generated_at: new Date().toISOString() });
    ...
    sendJson(response, 500, { code: "SERVER_ERROR", message: error.message });
  }
});
