...
export async function createStore(backendDir) {
  ...
  function recordUsage(event) {
    db.prepare("INSERT INTO usage_events (app_id, endpoint, status, latency_ms, created_at) VALUES (?, ?, ?, ?, ?)").run(event.app_id || "yingge-h5", event.endpoint, event.status, event.latency_ms, event.created_at || new Date().toISOString());
  }
  function recordInteraction(event) {
    ...
    db.prepare("INSERT INTO chat_interactions (message_id, app_id, question, answer, intent, quality, citation_count, citation_files, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(message_id) DO UPDATE SET answer=excluded.answer, intent=excluded.intent, quality=excluded.quality, citation_count=excluded.citation_count, citation_files=excluded.citation_files, status=CASE WHEN chat_interactions.rating='down' THEN 'needs_review' ELSE excluded.status END")
      .run(String(event.message_id), event.app_id || "yingge-h5", String(event.question || "").slice(0, 8000), String(event.answer || "").slice(0, 30000), event.intent || "general", quality, Number(event.citation_count || 0), JSON.stringify(Array.isArray(event.citation_files) ? event.citation_files : []), status, event.created_at || new Date().toISOString());
  }
  ...
  function listInteractions(status = "review") {
    const select = "SELECT message_id, app_id, question, answer, intent, quality, citation_count, citation_files, status, rating, feedback_reasons, feedback_note, created_at, feedback_at, reviewed_at FROM chat_interactions";
    let rows;
  ...
  function metrics() {
    const raw = db.prepare("SELECT COUNT(*) AS requests, SUM(CASE WHEN status < 400 THEN 1 ELSE 0 END) AS successes, SUM(CASE WHEN status >= 400 THEN 1 ELSE 0 END) AS errors, ROUND(AVG(latency_ms), 1) AS avg_latency_ms FROM usage_events").get();
    const summary = { requests: Number(raw.requests || 0), successes: Number(raw.successes || 0), errors: Number(raw.errors || 0), avg_latency_ms: Number(raw.avg_latency_ms || 0) };
    const byApp = db.prepare("SELECT app_id, COUNT(*) AS requests, SUM(CASE WHEN status < 400 THEN 1 ELSE 0 END) AS successes, ROUND(AVG(latency_ms), 1) AS avg_latency_ms FROM usage_events GROUP BY app_id ORDER BY requests DESC").all();
