...
if(document.querySelector('#collections .section-title')&&!document.querySelector('.origin-entry'))document.querySelector('#collections .section-title').insertAdjacentHTML('beforeend','<a class="origin-entry" href="content.html#origin"><span>先看英歌从哪里来</span><small>起源与流变 · 多种线索，保留证据边界</small><b aria-hidden="true">↗</b></a>');
if(document.querySelector('#coreExhibits')&&location.hash){requestAnimationFrame(()=>requestAnimationFrame(()=>{const target=document.getElementById(location.hash.slice(1));if(target)target.scrollIntoView({block:'start'});}));}
