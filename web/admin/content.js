(() => {
  ...
  async function load() { try { const response = await fetch(`${API}/api/admin/site-content`, {headers:headers()}); if (!response.ok) throw new Error('无法读取内容配置'); fill(await response.json()); } catch { try { const cached = JSON.parse(localStorage.getItem('yingge-site-content') || 'null'); if (cached) fill(cached); } catch {} } }
