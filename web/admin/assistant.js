(() => {
  ...
  const freechat = document.createElement('link'); freechat.rel = 'stylesheet'; freechat.href = 'assistant-freechat.css'; document.head.appendChild(freechat);
  ...
  form.addEventListener('submit', async event => {
    ...
    const prompt = `你是英歌舞数字博物馆的智能助手。请先自动判断本轮需求属于知识问答、资料解释、内容编辑、项目策划或其他任务，再用最合适的方式回答。需要方案时，给出目标、受众、内容结构、执行步骤、资源需求、风险与需要核实的事实；涉及馆内事实时附[证据N]引用；不要把单一地区、队伍或版本泛化为全部英歌舞。以下是最近对话上下文，请承接但不要重复：\n${recent}\n\n本轮需求：${text}`;
    try {
      const response = await fetch(`${API}/api/admin/test`, {method: 'POST', headers: {'Content-Type': 'application/json', ...tokenHeaders()}, body: JSON.stringify({app_id: 'yingge-h5', message: prompt, context: {audience: '自动判断', detail_level: 'deep', source_mode: 'brief'}})});
