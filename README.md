...
- 英歌小槌本地知识库问答接口
- 团队视频与木疙瘩 H5 的明确接入位置
- B 端可视化运营台（总览、洞察、审核、策划、评测）
- 独立的英歌小槌对话工作区（从运营台侧栏进入，生成/优化项目方案）

后台当前包含一套位于 `web/admin/` 的可视化运营台，以及受保护的管理 API，覆盖反馈、知识任务、答案修订、证据审计、评测和应用注册。打开 http://127.0.0.1:8096/admin/ 即可进入运营台。旧工作区的管理页面没有迁入。

前台内容编辑入口为 http://127.0.0.1:8096/admin/content.html。它支持首页 Hero 与三项核心展项的表单编辑、公众前台实时预览、保存发布和本机草稿回退；视频与木疙瘩页面先保留接入位。英歌小槌独立入口为 http://127.0.0.1:8096/admin/assistant.html，是后台中的额外对话工作区；生成的方案会自动同步到 http://127.0.0.1:8096/admin/plans.html 方案记录，不替代总览、内容审核或知识评测模块。
